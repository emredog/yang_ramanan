...
for rlevel = levels,
  for c  = 1:length(model.components),
    ...
    %thresh = -100.0;
    [Y,X] = find(rscore >= thresh); %finds indices of rscore elements that are >= threshold
    if length(X) > 1,
...
function box = backtrack(x,y,mix,parts,pyra)
  ...
  for k = 1:numparts,
    curPart   = parts(k);
    if k == 1, %If current part is root:
      xptr(:,k) = x;
      yptr(:,k) = y;
      mptr(:,k) = mix;
    else %if current part is any other part than root
